...
# CHECK WHETHER NECESSARY TO RECALCULATE - always do for now.
""" 
try:
    st=os.stat(path_save_cntry+'cis_nw_nodes')    
    mtime=st.st_mtime
    modified = datetime.fromtimestamp(mtime, tz=timezone.utc)
    pass_date = datetime(2022,12,15, 0,0,0,0,timezone.utc)
except FileNotFoundError:
    pass_date = datetime(2022,12,10, 0,0,0,0,timezone.utc)
    modified = datetime(2022,12,9, 0,0,0,0,timezone.utc)
"""
pass_date = datetime(2022,12,10, 0,0,0,0,timezone.utc)
modified = datetime(2022,12,9, 0,0,0,0,timezone.utc)
# only if network is older than pass-date
if modified < pass_date:
