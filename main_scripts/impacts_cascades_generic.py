...
class ImpFuncsCIRiverFlood():
    ...
    def step_impf(self):
        step_impf = ImpactFunc() 
        step_impf.id = 1
        step_impf.haz_type = 'RF'
        step_impf.name = 'Step function flood'
        step_impf.intensity_unit = ''
        step_impf.intensity = np.array([0, 0.95,0.955, 1, 10])
    ...
    def no_impf(self):
        ...
        no_impf.name = 'No impact function flood'
        no_impf.intensity_unit = ''
        no_impf.intensity = np.array([0, 1, 10])
        no_impf.mdd =       np.array([0, 0, 0])
        no_impf.paa =       np.sort(np.linspace(1, 1, num=3))
        no_impf.check()
        return no_impf
...
def ImpfClassDict():
    return {
        ...
        'TC' : ImpFuncsCIWind(),
        'EQ' : ImpFuncsCIQuake(),
        'RF' : ImpFuncsCIRiverFlood()
    }
# =============================================================================
# Hazard loading funcs
# =============================================================================

def get_selected_rf(iso3, path_flood, gcm='gfdl-esm2m', ghm='h08', rcp='historical_histsoc', years=np.arange(1971,2006)):
    
    dph_path = f'{path_flood}flddph_{gcm}_{ghm}_{rcp}_flopros.nc'
    frc_path = f'{path_flood}fldfrc_{gcm}_{ghm}_{rcp}_flopros.nc'
    rf = RiverFlood.from_nc(countries =['CHE'], years=years, dph_path=dph_path, frc_path=frc_path)
       
    # only nonzeros within country / state shape
    # and only reasonably strong ones
    rf.centroids.set_geometry_points()
    inten_selectors = []
    for event_id in rf.event_id:
        bools_1 = rf.select(event_id=[event_id]).intensity>1
        if bools_1.sum()>10:
            inten_selectors.append(event_id)
    rf = rf.select(event_id=inten_selectors)
    return rf
...
def get_selected_tcs_api(iso3, state_shape, start=START_STR, end=END_STR):
    ...
    for event_id in tc.event_id:
        ...
        if bools_30.sum()>0:
            ...
            if sum(tc.centroids.geometry[bools_30].within(cntry_shape))>0:
                inten_selectors.append(event_id)
...
def wrapper_impacts_cascades_saving(haz, ci_network, exp_list, df_dependencies, friction_surf):
    ...
    haz: Hazard
    ci_network : Network
    exp_list : [exp_health, exp_educ, exp_celltowers, exp_pplant, exp_pline_n, exp_pline_e, exp_ptower, 
                exp_road, exp_people]
...
if __name__ == '__main__': 
    ...
    if not os.path.exists(path_save+'old/'):
        os.makedirs(path_save+'old/')
    ...
    if haz_type=='FL':
        ...
        hazards = get_selected_tcs_api(iso3, cntry_shape)
    
    elif haz_type=='RF':
        path_haz=path_save+f'{cntry.lower()}_isimip2b/'
        hazards = get_selected_rf(iso3, path_haz)
    ...
    # line exposures    
    res_orig = 500
    disagg_val_pline = 500 if haz_type in ['FL', 'RF'] else 1 # damage fraction on y-axis for FL, failure prob on y-axis for EQ and TC
